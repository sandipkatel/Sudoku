...
void Sudoku::handleFile::writeToFile(handleFile hf)
{
...
void Sudoku::handleFile::setData(int timeElapshed, std::string bestTime, std::string username, Buttons Level)
{
    mTime = timeElapshed;
    mBestTime = bestTime;
...
int Sudoku::handleFile::getData(std::string (&myData)[], Buttons home)
{
    // Open the file
...
void Sudoku::handleFile::getData(Buttons Level, std::string &bestTime, std::string &bestPleyer, int &Time)
{
    if (Level == Buttons::HARD)
        mLevel = "Hard";
    else if (Level == Buttons::MEDIUM)
