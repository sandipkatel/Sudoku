...
bool Sudoku::Sudoku::initialiseSDL()
{
	...
	//Load splash image
    mImageSurface = IMG_Load("Dependencies/Sudoku.bmp");
	...
	// Load font for text
	mFont = TTF_OpenFont("Dependencies/arial.ttf", mFontSize);
	if (mFont == nullptr)
	{
...
void Sudoku::Sudoku::backButton()
{
	...
	int y = buttonHeight;
	SDL_Texture* texture;
	loadTexture(texture, "Back", BlackColor);
	SDL_Rect buttonRect = { x, y, buttonWidth, buttonHeight };
	mBack.renderRectangle(mRenderer, buttonRect, draw);
	mBack.renderTexts(mRenderer, buttonRect, texture, 2);
}
...
Buttons Sudoku::Sudoku::play(Buttons level)
{
	...
	while (!stop)
	{
		while (SDL_PollEvent(&event) != 0)
		{
			...
			//check if Back is clicked or not
		...
		for (int cell = 0; cell < mTotalCells; cell++)
		{
			...
			//Set incorrect if reached count equal to five
			if(hintCount == 5)
			{
				mHintButton.setIncorrect(true);
			}
