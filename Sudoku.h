...
void Sudoku::Sudoku::preloadTextures()
{
	...
	loadTexture(mTextureCache[14], "Hint", fontColour);
	loadTexture(mTextureCache[15], "SUDOKU", fontColour);
	loadTexture(mTextureCache[16], "Exit", fontColour);
	loadTexture(mTextureCache[17], "Username", fontColour);
	loadTexture(mTextureCache[18], "Best Score", fontColour);
...
void Sudoku::Sudoku::createInterfaceLayout()
{
	...
	//Define start state
	int buttonStartRow = 0;  // s down
	int buttonWidth = mGridWidth - 2 * thickBorder;   //s just timers button width
	// mWindowHeight = buttonHeight + 6 * thinBorder + 6 * thickBorder (rearange this equation)
	int buttonHeight = (mWindowHeight - 6 * thinBorder - 6 * thickBorder-30) / 11;  //s cells height
	...
	bool fill = 1, draw = 0;  // to draw or fill
	//display the background of Sudoku black
	SDL_SetRenderDrawColor(mRenderer, 255, 255, 255, 255);
	mRectangle.renderRectangle(mRenderer,0, 0, mGridWidth, mGridHeight, fill);
