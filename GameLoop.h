...
void Sudoku::Sudoku::display()
{
    ...
    clicked = homeScreen();
    //clicked = home_levels(Buttons::HOME);
    while(!stop)
    {
        switch(clicked)
        {
        ...
        case Buttons::LEVEL:
            clicked = levels();
            //clicked = home_levels(Buttons::LEVEL);
            break;
        case Buttons::HARD:
...
Buttons Sudoku::Sudoku::homeScreen()
{
    ...
    //load START, ABOUT and BEST texture
...
Buttons Sudoku::Sudoku::userName()
{
    ...
    //Load and Display User name and Enter Your name here Textures
    ...
    bool stop = false;
    bool exit = false;
    bool userEntered = false;  //Check if User enters the enter key of not
    while(!stop)
    {    
        //Handle events
        SDL_Event event;
